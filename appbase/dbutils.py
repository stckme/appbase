...
def drop_empty_create_table(model):
    ...
    if model.table_exists():
        ...
        if result and result[0] and result[0][0]:
            logger.error(
                "%s table already exists and has data, cant be dropped", table_name
            )
    ...
    if model.table_exists():
        ...
        if result and result[0] and result[0][0]:
            ...
            try:
                ...
                logger.info(
                    "%s column is added to the table %s", column_name, table_name
                )
            except Exception as e:
                db.rollback()
                logger.error("%s: %s", type(e).__name__, e)
    ...
    if model.table_exists():
        ...
        if a and a[0] and a[0][0]:
            try:
                ...
                logger.info(
                    "%s column is removed from the table %s", column_name, table_name
                )
            except Exception as e:
                db.rollback()
                logger.error("%s: %s", type(e).__name__, e)
            ...
            logger.info(
                "%s column is not present in the table %s", column_name, table_name
            )
    else:
        logger.error("No table with name %s found", table_name)
