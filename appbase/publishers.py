...
def extract_kw(request):
    return (
        (request.args and dict((k, v) for (k, v) in request.args.items()))
        or request.json
        or (request.data and json.loads(request.data.decode("utf-8")))
        or request.form
        or {}
    )
...
def protected(f):
    ...
    def wrapper(*args, **kw):
        session_id = (
            kw.pop("_session_id", None)
            or hasattr(context.current, "sid")
            and context.current.sid
        )
        ...
        if roles_required and not set(context.current.groups).intersection(
            roles_required
        ):
...
class HTTPPublisher(object):
    ...
    def add_mapping(self, url, handler, methods=["GET"], jsonify_result=True):
        ...
        add_url_rule(
            self.app, url, handler, methods=methods, jsonify_result=jsonify_result
        )
